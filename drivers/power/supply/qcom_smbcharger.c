...
static int smbchg_usb_set_ilim_hc(struct smbchg_chip *chip, int current_ua)
{
	 ...
	 */
	ilim_index = find_smaller_in_array(chip->data->ilim_table,
					chip->data->ilim_table_len,
					current_ua);
...
static int smbchg_charging_get_ilim(struct smbchg_chip *chip)
{
	 ...
	 */
	ret = regmap_read(chip->regmap,
			  chip->base + SMBCHG_CHGR_FCC_CFG, &value);
	if (ret)
		return ret;
...
static int smbchg_charging_set_ilim(struct smbchg_chip *chip, int current_ua)
{
	...
	/* Set the current limit index */
	ret = smbchg_sec_masked_write(chip, SMBCHG_CHGR_FCC_CFG,
				FCC_MASK, ilim_index);
	if (ret) {
		dev_err(chip->dev,
...
static int smbchg_init(struct smbchg_chip *chip)
{
	...
	ret = smbchg_charging_set_ilim(chip,
	...
	/* Set float voltage */
	ret = smbchg_charging_set_vfloat(chip, chip->batt_info->voltage_max_design_uv);
	if (ret) {
		dev_err(chip->dev,
...
static int smbchg_probe(struct platform_device *pdev)
{
	...
	if (chip->batt_info->constant_charge_current_max_ua == -EINVAL) {
		dev_err(chip->dev, "Battery info missing maximum constant charge current\n");
		return -EINVAL;
	}

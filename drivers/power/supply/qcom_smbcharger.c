...
irqreturn_t smbchg_handle_usbid_change(int irq, void *data)
{
	...
	if(smbchg_is_otg_present(chip)) {
		dev_info(chip->dev, "OTG present\n");

		ret = regulator_enable(chip->otg_supply);
		if(ret) {
			dev_err(chip->dev, "Faild to enable OTG VBUS supply: %d\n", ret);
			return IRQ_NONE;
		}
	} else {
		dev_info(chip->dev, "OTG not present\n");
 
		regulator_disable(chip->otg_supply);
	}
...
static int smbchg_probe(struct platform_device *pdev)
{
	...
	chip->otg_supply = devm_regulator_get(&pdev->dev, "otg");
	if(IS_ERR(chip->otg_supply)) {
		ret = PTR_ERR(chip->otg_supply);
		dev_err(chip->dev, "failed to get OTG VBUS regulator: %d", ret);
		return ret;
	}
